...
export default function OptionModal({ del,loanId }) {
    ...
    const getProducts = (id) => {
      ...
      const approveLoan = async (values) => {
        try {
            setDelBtn(true);
            console.log(values);
            ...
            if (api.isSuccessful(res)) {
              setTimeout(() => {
                trigger("reRenderPendingCustomerLoan")
                handleClose()
  ...
  const deleteLoan = async (values) => {
    ...
    return (
        <div>
            <Button style={{ color: 'black', textTransform: 'none' }} onClick={handleOpen && handleProps} >{del ? `Deny` : `Approve`}</Button>
            <Modal
