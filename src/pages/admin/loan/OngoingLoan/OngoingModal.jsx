...
import { Button, Divider, FormControl, IconButton, InputLabel, MenuItem, Paper, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from "@mui/material";
...
export default function OptionModal({ history, loanId }) {
    ...
    const loanHistory = async () => {
        ...
        const loans = await api
            .service()
            .fetch(`/dashboard/transactions/?loan=${loanId}?limit=${limit}&offset=${offset}`, true);
        console.log(loans.data)

        if ((api.isSuccessful(loans))) {
            setLoanHist(loans.data.results);
            setCount(loans.data.count)
            setIsLoading(false)
        }
        setIsLoading(false)
