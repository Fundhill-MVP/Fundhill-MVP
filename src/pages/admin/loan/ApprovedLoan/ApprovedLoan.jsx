...
function ApprovedLoan() {
  ...
  const ApprovedCustomerLoan = async () => {
    setIsLoading(true)
    const res = await api.service().fetch(`/dashboard/loan/?status=APPROVED?limit=${limit}&offset=${offset}`, true);
  ...
  on("reRenderApprovedCustomerLoan", ApprovedCustomerLoan)
