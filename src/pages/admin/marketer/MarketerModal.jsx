...
export default function ActionButton({ del, setCurrentId }) {
    ...
    useEffect(() => {
        ...
        const allMarketer = async() => {
          const res = await api.service().fetch(`/accounts/manage/?is_staff=True&id=${setCurrentId}`,true);
        ...
        try {
            ...
            if(api.isSuccessful(res)){
                setTimeout( () => {
                    ...
                    trigger("reRenderDisbledMarketers");
                    setActiveBtn(false);
                    handleClose()
                    toast.success("Successfully activated marketer!");
                },0);
                }
                setActiveBtn(false);
        } catch (error) {
                ...
                <Fade in={open}>
                    <Box sx={style}>
                        ...
                        {activeBtn ? (
                            ...
                            :
                             (
                                <Button onClick={() => activateCustomer(setCurrentId)} variant="contained" style={{ textTransform: 'none', marginRight: 5 }} >Activate</Button>
                             )
                             }
                             {deactiveBtn ? (
                             <CircularProgress size={26} />
                            ...
                            :
                             (
                                <Button onClick={() =>  deactivateCustomer(setCurrentId)} style={{ background: 'gray', color: 'white', textTransform: 'none' }} >Deactivate</Button>
                             )
                             }
                             {delBtn ? (
                             <CircularProgress size={26} />
                            ...
                            :
                             (
                                <Button onClick={() => deleteCustomer(setCurrentId)} style={{ background: 'red', color: 'white', marginLeft: 5, textTransform: 'none' }}>Delete</Button>
