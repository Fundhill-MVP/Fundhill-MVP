...
export default function OptionModal({ del, productId }) {
    ...
    const editProduct = async (values, id) => {
        setBtnLoading(true);
            ...
            const response = await api
                  .service()
                  .update(`/dashboard/savings-plan-type/${id}/`,values,true)
    
            if(api.isSuccessful(response)){
              setTimeout( () => {
                handleClose()
                trigger("reRenderSavingsType")
                toast.success("Savings product successfully updated!");
              },0);
            setBtnLoading(false);
        }
        setBtnLoading(false)
    ...
    const deleteProduct = async (id) => {
        try {
            setDelBtn(true);
            const res = await api.service().remove(`/dashboard/savings-plan-type/${id}/`, true);
            console.log(res.data)
            if (api.isSuccessful(res)) {
                setTimeout(() => {
                    handleClose()
                    toast.success("Successfully deleted Savings Product!");
                    trigger("reRenderSavingsType")
                    setDelBtn(false)
                }, 0);
                setDelBtn(false)
                        ...
                        {del ? (
                            <>
                                <Typography sx={{ mt: 2, mb: 2, fontWeight: 600 }} variant="h6" component="h5" gutterBottom>
                                    Confirm delete of Savings Type Product
                                </Typography>
                                ...
                                <Typography id="transition-modal-description" sx={{ mt: 2, mb: 2 }} gutterBottom>
                                    Are you sure you want to delete this Savings type Product?
                                </Typography>
                            ...
                            (
                                <>
                                    ...
                                    <Typography sx={{ mt: 2, mb: 2, fontWeight: 600 }}>
                                        Edit Savings Type Product
                                    </Typography>
                                    <Formik
                                        initialValues={{
                                            name: item?.name,
                                            marketer_earnings: item?.marketer_earnings,
                                            number_of_cycles_to_charge: item?.number_of_cycles_to_charge,
